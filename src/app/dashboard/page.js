...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (accessToken) {
        ...
        await axios
          .get(`${API_BASE_URL}/api/employee/activity/topApplications`, {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          })
        ...
        await axios
          .get(`${API_BASE_URL}/api/employee/activity/topSites`, {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          })
          ?.then((res) => {
            console.log(res);
            setTopWebsites(res?.data?.data);
          })
          ?.catch((err) => {
            console.error(err);
          });
        await axios
          .get(`${API_BASE_URL}/api/employee/activity/getUncategorizedData`, {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          })
          ?.then((res) => {
            console.log(res);
            setPendingClassifications(res?.data?.data);
          })
          ?.catch((err) => {
            console.error(err);
          });
  ...
  return (
    <div className="h-fit flex-1 flex flex-col gap-10 px-10 pb-20 pt-5">
      <DashboardHeader handleRefresh={handleRefresh} />
      <div className="w-full grid grid-cols-2 gap-10">
        <ProductivityChart productivity={productivity} />
        <TopUsersAndGroupsCharts topUsersAndGroups={topUsersAndGroups} />
